...
WINDOWS = {
    ...
    'tod':  [('A', 3.40), ('B', 3.45), ('C', 3.30), ('D', 3.20), ('E', 4.50),
             ('F', 2.90), ('G', 3.30)],
    # DC-path news reels (2026-08-25 pacing law): a story carrying a comparison AND a
    # catch runs six or seven beats at ~2.8s. Added 2026-08-27 after a 19.6s seven-beat
    # cut was written with no window map at all - scene count alone would have inferred
    # 'tod' and graded it against 24.05s of windows, i.e. 23% MORE time than the cut has,
    # which is the exact failure this table's neighbour comment warns about. A spec for
    # these MUST carry "engine": "news6" / "news7"; scene count cannot disambiguate.
    'news6': [(c, 2.8) for c in 'ABCDEF'],
    'news7': [(c, 2.8) for c in 'ABCDEFG'],
}
WARN, FAIL = 3.6, 4.2
A_FAIL = 4.7          # scene A ceiling - the loud open
MAX_CLAUSES = 2
...
def main():
    ...
    # count-inference is only safe where a length maps to exactly ONE engine
    _lens = {}
    for k, v in WINDOWS.items():
        _lens.setdefault(len(v), []).append(k)
    by_count = {n: ks[0] for n, ks in _lens.items() if len(ks) == 1}
    ...
    print(f"gate 8 · VO budget  ({engine} engine, warn >{WARN}, fail >{FAIL} syl/sec; scene A fails >{A_FAIL}; max {MAX_CLAUSES} clauses)")
    print(f"  {'scene':<14}{'window':>8}{'words':>7}{'syl':>6}{'syl/sec':>10}{'w/sec':>8}{'cl':>4}  ")
    tot_w = tot_s = tot_d = 0
    worst, bad = 0.0, []
    for (sid, dur), sc in zip(win, scenes):
        ...
        fail_at = A_FAIL if sid == 'A' else FAIL
        flag = 'FAIL' if rate > fail_at else ('warn' if rate > WARN and sid != 'A' else '')
        clauses = len([c for c in re.split(r'[.!?]+', sc.get('line', '')) if c.strip()])
        if clauses > MAX_CLAUSES:
            flag = (flag + ' CLAUSES').strip()
        if flag:
            bad.append((sid, rate, flag))
        wps = w / dur if dur else 0
        print(f"  {sid + ' · ' + sc.get('id','')[:9]:<14}{dur:7.1f}s{w:7}{s:6}{rate:10.1f}{wps:8.1f}{clauses:4}  {flag}")
    overall = tot_s / tot_d if tot_d else 0
    print(f"  {'TOTAL':<14}{tot_d:7.1f}s{tot_w:7}{tot_s:6}{overall:10.1f}{(tot_w/tot_d if tot_d else 0):8.1f}")

    if any('FAIL' in f for _, _, f in bad):
        print(f"\nFAIL — {', '.join(s for s,_,f in bad if 'FAIL' in f)} cannot be read in the time. Cut words.")
        return 1
    if any('CLAUSES' in f for _, _, f in bad):
        print(f"\nCLAUSES — {', '.join(s for s,_,f in bad if 'CLAUSES' in f)}: more than two sentences in one beat. One clause per beat, max two.")
        return 1
    if bad:
