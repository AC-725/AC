...
## OPEN STING — the hand-off law, built (open_sting.html, 2026-08-24)

First lab file that obeys the hand-off law end to end: **particles carry the MOVE, a
drawn stroke carries the HOLD.** seek(t)-driven at 1080x1920 native, so
`render_frames.js` bakes it with no special casing.

**Why native size matters.** The shipped vault open puts the 720x792 art into a 224x246
box (`reel.template.html` `#markopen`). That is a 3.2x NON-INTEGER downscale of a 1px dot
field, and resampling it differently every frame is what makes the open sparkle and crawl.
Rendering the sting at the size it is displayed removes the resample entirely.

**The build, and the four rules it had to learn the hard way (all already in
`references/site.md`, all of them re-broken here first):**
- *A mark of grains is grain.* The settle is NOT denser particles - the cloud gathers,
  then an inline-SVG mark inks itself on by `stroke-dashoffset`, one `<path>` per stroke
  because Chrome resets dashing per subpath, each window from a real `getTotalLength()`.
- *Never clamp particles into a safe box, GENERATE them inside it.* A clamp on the start
  radius stacks every outlier on one circle and lays a hard gold rim - exactly the
  hard-edged dust rectangle this was meant to remove. Now `r = RMAX*pow(u,1.35)`,
  centre-heavy, thinning to nothing, with RMAX derived from the frustum
  (halfWidth = 6.6*tan(25deg)*(1080/1920) = 1.731, so RMAX 1.45 leaves ~78px).
- *`fract(sin(i)*k)` aliases.* It also aliases when two seeds are merely ADJACENT: feeding
  h32 `i*2+1` and `i*2+2` for radius and angle laid visible spiral arms through the cloud.
  Separate multipliers plus XOR salts.
- *PointsMaterial draws hard squares*, and gold dimmed toward black on a near-black ground
  reads as dirt. So: radial-gradient CanvasTexture, NormalBlending, and a small
  ShaderMaterial carrying per-vertex ALPHA, because PointsMaterial can only fade through
  colour and colour is the wrong channel here.

**Two things this file adds that were not written down anywhere yet:**
- **A global fade cannot honour the hand-off.** The cloud covers the whole mark, so while
  the A is already drawn the C's grains have nothing to hand to, and a dense cloud held at
  15-35% reads as an olive ghost. Each grain is tagged with the stroke that owns it
  (strokes rasterised in paint order, later ones claiming overlaps) and dissolves on THAT
  stroke's progress.
- **Even per-stroke is not enough.** Fading a stroke's grains on its global progress
  empties the whole C at once and opens a dark gap ahead of the pen. Each grain also
  carries its position ALONG its stroke (nearest segment on the polyline) and hands over
  only once the draw front passes it, so grains dissolve INTO the line at the point the
  line reaches them.

Measured on the render, not judged by eye: HOLD mark x 212..869 y 359..1080, centre within
0.5px of frame centre, inside the app-safe window (170 < y < 1300) - it needed
`MARK_Y=0.77` to get there, since at dead centre it ran to y1359, 59px into Instagram's
caption band. Frame-0 cloud has zero ink in the outer 8px on any edge.

NEXT on GO: bake at the display size and wire behind the vault theme's `#markopen`, or
carry the same particles-move / stroke-holds structure into the scene-E sign-off. Both
still need the full gate battery and the 140px test.

## PATH REPAIR (2026-08-24)

Every file in `theme-candidates/` was DEAD: the importmap resolved `./three.module.min.js`
against `theme-candidates/`, where three does not live, so `open_draft.html` rendered zero
canvases and never reached `document.title='ready'`. The Poppins and Gloock `@font-face`
rules pointed at a `fonts/` dir that holds only the four new theme faces. 17 files
repaired to point at the real locations (`../three.module.min.js`,
`../../fonts/`). The PNG contact sheets in this dir predate the breakage, which is why it
went unnoticed - a sheet on disk is not proof its source still runs.
