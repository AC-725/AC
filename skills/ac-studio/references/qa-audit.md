...
## Gate 7 · the filmstrip (added 2026-08-18, Day 35)

```bash
bash SKILL_DIR/scripts/qa_filmstrip.sh <engine>.html    # after qa_layout, before produce
```

**The blind spot it closes.** Gates 1, 2 and 6 freeze each scene at `start + dur*0.55`
via `audit_capture.js` — a SETTLED moment. Gates 3, 4 and 5 measure the cut, not the
picture. So anything wrong only *while* something animates was invisible to all six:
a counter mid-count, a rule mid-draw, an icon mid-swap, a scene that has not composed
yet. That is most of the running time of every video, and on Day 35 it hid five real
defects at once — a hero figure with no unit, a price odometer showing prices the vendor
never charged, a strike drawn across the wrong object, and two scenes that held a lone
element over a hole for nearly two seconds.

**What it does.** Samples every scene at four phases — entry, **in flight**, settled,
exit — plus both sides of every cut, reading the timings from the page's own `SCENES`
array so it works on any engine. Tiles them into `_filmstrip/FILMSTRIP.jpg`, timestamp
burned onto each tile, and prints the map.

**It returns no verdict, and that is deliberate.** Nothing mechanical can tell you a
number looks wrong or a swap reads as two events. The gate is the READING. Go tile by
tile and ask:

- does every figure carry its unit **while it counts**, and is every intermediate value
  a true one (a tally may count; **money may not**)
- does every drawn rule, strike or underline stay inside the thing it marks
- does every swap read as one move rather than two events fighting
- is any frame a lone element stranded over a hole — a scene that composes late
- is the picture composed on **both sides** of every cut
- does anything mid-type leave a line unreadable or a word orphaned

Run it on every engine, every time, before `produce.sh`. A render whose filmstrip has
not been read is not ready.

## The toolchain resolves itself (2026-08-18)

Three of the scripts died mid-run on a fresh shell that day: `playwright` is installed in
the **global** node root, so a bare `require("playwright")` from a run directory throws
`MODULE_NOT_FOUND`; and this container ships no system `ffmpeg` (the one bundled with
Playwright is VP8/webm only, useless for an H.264 reel). Both had been working only because
an earlier shell happened to carry the right `NODE_PATH` and `PATH`.

`qa_layout.sh`, `qa_filmstrip.sh`, `produce.sh` and `build_video.sh` now resolve both for
themselves at the top of the file: `npm root -g` for node, and the static H.264/AAC build
from the python `imageio-ffmpeg` wheel for video, symlinked into `/root/bin` alongside the
small `ffprobe` shim the verify step calls. **A QA script that only runs in the shell that
happened to set it up is not a gate.** If a script grows a new external dependency, resolve
it in the script.

## What gate 4 can and cannot see

The motion floor measures the mean change over the whole 1080x1920 frame at -50 dB. On this
brand's flat black ground, only large bright events clear it — a hero number ticking (about
-45 dB), a browser card landing. Typed text, a blinking cursor, a 200px tile popping and the
slow push-in all read as motion to a viewer and all sit below the floor (-58 to -70 dB
measured). So a gate-4 stretch is a question, not a verdict: **read those seconds on the
filmstrip before you believe them.** If the frame genuinely holds a still picture, fix the
beat map (see `tod-scenes.md`); if it is alive and merely quiet, log the stretch with its
measured length and ship. Never add motion that says nothing just to clear the number.
