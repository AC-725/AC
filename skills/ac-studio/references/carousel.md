...
> **Read `carousel-components.md` first (2026-09-12).** Every carousel now opens with one
> AskUserQuestion form (theme · icon per slide · figure per slide · save slide · accent ·
> keyword), then a 50% stills board, then nothing renders full-size until AC approves the
> board. That file carries the crest carousel component vocabulary AC approved on the Two
> Counts deck and the layout gate that runs before the board ships.
