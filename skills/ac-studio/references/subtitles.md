...
pass then covers caption ink, and `produce.sh` builds the subtitled cut (the ONLY cut
by default — AC's one-cut rule, 2026-08-21; `--clean-also` still strips `SUBS`
automatically when a clean cut is explicitly asked for). What stays LAST is verification:
...
Ship the subtitled cut. It is the deliverable — the only one, since AC's one-cut rule
(2026-08-21). A clean cut exists only behind `--clean-also` on the day AC asks.
