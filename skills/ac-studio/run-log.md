...
|---|---|---|---|---|---|---|---|---|---|
| 2026-09-13 | 42 | AI News (18.6s, six beats, crest, DC engine) — **"76 / 14"** | Goldman Sachs 10,000 Small Businesses Voices: 76% of small businesses surveyed use AI, only 14% "fully integrating AI into core operations". Toolbox → workbench. GS wordmark ink-on-gold in the crest band as a SOURCE lockup (law 8). Cover built as its own static DC at 1080×1920, never a frozen animation frame (law 9). Captions restyled through the engine's `style` prop — the default Inter would have shipped silently (law 7). Keyword GOLD, same as Saturday. | **PRIMARY:** Goldman Sachs press release 17 Mar 2026, n=1,256, Babson College + David Binder Research, fieldwork 27 Jan–4 Feb 2026 — filed with all wording gates at `references/verified-numbers.md`. Footnote date Mar 2026, never Sep; sample is 10KSB alumni so never "of all small businesses" | crest · dark · GS wordmark in band | — | — | Claude Design project "Weekend content preparation" — not in package | — |
| 2026-09-12 | PD 09 | Saturday Prompt Drop (**7 slides + Story**) — **PROMPT DROP v5 BORN: the exchange is a SKILL** | **"Workflow Builder"** — three prompts on the slides for the interested, and a downloadable skill that writes the reader's own prompts for everything else. AC's call mid-interview; pack shape drops 9 → 7 (intro → rule → 3 prompts → the skill → close). **AC wrote the final skill himself** — the agent's draft was a scaffold and was thrown away, per the new law. His 4-file pack (READ ME FIRST · paste anywhere.md with a BRANDING switch · workflow-builder.skill · owner's guide) is what ships; the DM sends the paste file, Claude-skills users get the .skill. **Wireframe governed the icons because AC attached a reference sheet** — reference law beat the 9 Sep polished precedence; five bakes (gear, geartrain, fanoffive, speechbubble, docstack), bevels OFF, NoToneMapping, alpha ×1.45. v2 redesign the next day added the RED rule slide, polished-yellow 3D kickers on every sub-head, and the Instagram action pack as a real asset. Overlap gate caught three collisions the builder missed. Keyword GOLD. | same Goldman number as Day 42 | base · red rule slide · wireframe icons | hook: "Why is AI still not part of your actual work?" | — | `assets/ig/*.png` (5, shipped here) · `assets/icon-boards/finish-line-icon-board.png` · `references/prompt-drop-skill-exchange.md` · `references/verified-numbers.md` · pack + boards in Claude Design | — |
| 2026-09-12 | — | **CAROUSEL COMPONENTS — the Two Counts law folded in** (AC's update pack from a Claude Design build; deck itself not logged here) | "Two Counts" deck: 7 slides, crest, Claude Design, a two-count contrast (loud claim vs real count) with the approved red pair. Folded in: the one-form interview before every carousel (theme · icon per slide · figure per slide · save slide · accent · keyword) → 50% stills board → stop; the crest carousel component vocabulary (gradient band 0–88, hairline frame, kicker 72px, counter top-right, handle bottom-centre, 7-node spine 148px above floor, footnote every slide, Bricolage 800 heroes to 236px, headline caps 80–124 ≤3 lines, body 32px, lifted checklist rows, exactly one ink-on-gold slide, exactly one save bookmark, figure ~420px beat slides only, icons 260–400 top-right, close = diamond mark); the pre-board layout gate (7 checks); the Two Counts icon map (hourglass · datachart · documentstack · globe · shield · mark). Reconciled: gradient on the band only, red pair is a bounded hero-number exception, akimbo PNG = approved figure transform-only, never draw icons in SVG, carousels wireframe or matte per interview. **Not in package:** Stills Board .dc.html, Caption and DM md, ac-figure-akimbo.png, assets/icons/*.png, bake/icons.html, bake/ac-icons.module.js. CLAUDE.md arrived later the same day → `references/carousel-house-rules.md` (numbers at 50% board scale). Fork check clean both times. | AC's uploaded update (2026-09-12) | crest · Claude Design | — | — | `references/carousel-components.md` (new) · `carousel.md` pointer · SKILL.md block, router row, four guardrails | — |
| 2026-09-11 | 41 | AI News (16.8s, 6 × 2.8, news6) — **Three labs locked the door** · built in Claude Design, AC's update pack folded in | OpenAI Astra (1 Sep): first to the "Critical" cyber tier, 100% ExploitBench, refuses 91.5% vs 59% (GPT-5.6 Sol), two zero-days in testing, Daybreak Blue testers only. Google Gemini 3.8 Flash Cyber (2 Sep): patches "in minutes," 650+ vetted partners, security teams only. Anthropic Mythos 5.1 (Sep): "strongest cyber capabilities of any model we've released," trusted access only; Fable 5.1 finds flaws, writes no exploits. Reframe: not "AI hackers are here" — all three said not for you; your login is still the door. **Killed from coverage:** Google "2.6x" (not in Google's post), Anthropic "sandbox escape / paused evals" (not in Anthropic's post). AC's row, verbatim: *column · 16.8s (6 × 2.8) · figure on · subs on · loop · gate 8 clean at 24 stamps, logo clearance ≥53px · anchor: matte padlock (new bake) · vendor logos on doors (AC's call) · headlines 3/5/6 agent-picked from offered options · VO cut to 2.4 syl/s after AC struck v1 · cover v2 with stage · keyword DOOR.* Approved VO measured here: 55 syl / 16.8s = 3.3 syl/s, 2.0 words/s — the "2.4" is words/s; gate 8 retuned to it. Twelve laws folded in (SKILL.md update 2026-09-11). Second option built the same day: the DeepMind whistleblower reel, now **Day 42, Friday**. Set aside: Anthropic resignation + Hubinger ">10%" (doom-adjacent), Navier-Stokes dispute (live), NSA China distillation (HK exposure, AC's call). | OpenAI openai.com/index/path-to-astra (1 Sep 2026) · Google blog.google Fairwind Program (2 Sep 2026) · Anthropic anthropic.com/claude-fable-and-mythos-5-1 (Sep 2026) — all read direct | column · dark · figure on · matte padlock anchor · vendor marks on doors | A (Authority bank: "Three labs, three hacking models, all locked up." → AC's final: "Three labs built hacking AI. Then locked it away.") | 9/10 | Claude Design project: reel-day41.jsx · Day 41 Reel/Cover/Stills/Bake .dc.html · export/ (not in package) · here: /root/runs/day41-locked/beatsheet.md, vo_spec.json, vo_final.json | — |
| 2026-09-09 | Prompt Drop 08 | **Prompt Drop v4 — the Claude Design engine folded in** (AC's standalone `ac-prompt-drop` SKILL.md, transferred) | Pack 08 **"The Edges"** built and shipped in Claude Design on the Pew number. Spec folded in verbatim at `references/prompt-drop-dc.md`; SKILL.md update block, router row, three guardrails. Laws: intro cover (heading 124px + pack name in gradient gold — the one gradient allowed — + hook question 44px + WORKS IN strip + hero icon 400px + five mini icons); rule slide carries the capstone icon at .85 + PASTE INTO strip; **prompt house style "You are X. Input. Output shape. Done line.", never "Act as"** (Pack 08's five rewritten in Claude Design; 4 Sep source md awaits final wording + REVISION line); **polished MeshPhysical pack icons** (roughness .14, metalness .95, clearcoat 1, PMREM, ACES; engraving #6E5626; 1024px `pdNN-<name>.png`) — third look, precedence set: polished → packs, matte → other carousels, wireframe legal; platform logos on 01 + 02 only, recoloured #C9A961; **Story cover (1080×1920) now a deliverable**; gate = bbox + intro hook in centre crop + 140px read, header overlap flag a known false positive. **PACKAGING REGRESSION #2, mine:** the 4 Sep package was built on a 1 Sep working copy and saved over a synced cache that already held the 2 Sep and 3 Sep updates. Four SKILL.md hunks restored from the 4 Sep session transcript (matte bakes + carousel Technique Day; tier list + iceberg + reference law + no-overlap gate + taste line; the static-formats section; six guardrails). NOT recovered: ten matte PNGs, the 2–3 Sep run-log rows (reconstructed below from SKILL.md, marked), any reference edits from those sessions. Fork rule amended: diff against the synced cache AS IT IS NOW before every zip. **Still missing from the package:** `templates/prompt-drop/Prompt Drop Template.dc.html`, `templates/prompt-drop/bake_icons.html`, `assets/logos/{chatgpt,claude,gemini}.png`, `assets/icons3d/pd08-*.png`, Pack 08 exports + story. | AC's uploaded SKILL.md (2026-09-09) | base · Claude Design | — | — | `references/prompt-drop-dc.md` (new) · SKILL.md · `references/prompt-drop.md` (v4 pointer) · `references/icons3d.md` (third look + matte-missing note) | — |
| 2026-09-03 | — | **RECONSTRUCTED FROM SKILL.md** (row lost in the 4 Sep regression) — Two static posts: **"The AI Efficiency Tier List"** (Post A, tier grid S→D) and **"How far down are you?"** (Post B, iceberg) | 1080×1350, base theme, built in Design Components. Tier grid: 190px tier column S bright → D dim (#E7C765 · #B08D3F · #796938 · #5E522C · #3A3218), Gloock 58px letters, 72px matte-gold object per tier (trophy → medal → ribbon → ticket → deflated balloon), vendor logos as cut-outs with no tile, 3 per row (4 in D), foot line is a hook ending in the ask. Iceberg: flat vector traced from AC's reference, 440px, sea line crown:body ≈ 1:3.5, crown #F4E7BF, ~25 body shards alternating #E7C765 / #C9A961 / #B08D3F / #9A7A36 / #8F7233, 1.25px plane hairlines, 1.5px outline, level chips left column only with 1.25px hairline glyphs (3D rejected at ≤60px), habit bullets ≤264px at 17px Poppins nowrap. Laws set in AC's words: **reference law** (an attached photo is the spec — measure it, re-open it every revision; a flat reference ships flat — nine 3D bakes rejected against a 2D illustration), **no-overlap gate on the builder** (bbox every image and text column before hand-off), **taste line** "directive, informative, innovative and minimalistic unique." | Gallup AI-at-work indicator: 52% / 30% / 15%, dated **May 2026** on the source page (not July — two boards shipped "Jul 2026" before the check) | base · dark | — | — | Pack, notes and CLAUDE.md in the project export (Claude Design) — not in this package | — |
| 2026-09-02 | Day 40 | **RECONSTRUCTED FROM SKILL.md** (row lost in the 4 Sep regression) — Technique Day as a CAROUSEL: **"Ask, don't tell"** (7 slides, DC path, no shell) | Same 7-beat spine as the reel (hook → answer → proof → catch → copy → rule → ask); one 3D anchor per slide at 208px lower-right, never in the headline band (top:252 collided with every headline wider than 640px, three boards running); chip hero 300px on the cover with a 3-icon proof rail at 128px under the stat line; walker on beat slides only (off cover, proof, copy — one gold mass); Instagram action icons as 1.4px gold hairline strokes, two on the cover (save lit), labelled four-up on the close (Comment KEYWORD lit). **THE MATTE LAW:** AC ruled in writing that the 30 Aug brand block (matte gold trio, soft warm key + gold rim, 3/4 front-left, soft contact shadow) governs in-house three.js bakes, superseding the 22 Aug wireframe-only ruling for icons; gold only, no cream, no black substrate; wireframe set stays legal. **Ten matte bakes:** chip, bubbles, scale, dial, key, question, messagestack, hourglass, shield, brain → `assets/icons3d/matte/` (**PNGs missing from this package**). Bake capture without a shell: alpha WebGL canvas with preserveDrawingBuffer → toDataURL → screenshot each solo at 600px on #0A0A0A → knock luminance ≤14 to alpha, ramp 14–40 (Gate 9 inverted for a black ground); edges carry a faint dark fringe, mount only on #0A0A0A. Bake page `ac-studio-lab/threejs-lab/bake_icons_day40.html` (pinned unpkg import map). | — (technique, no vendor number) | base · dark · matte anchors | — | — | Claude Design project — not in this package | — |
| 2026-09-04 | Prompt Drop 08 | Saturday Prompt Drop (9-slide, v3) — **CONTENT HAND-OFF to Claude Design** (design not built here) | **"The Edges"** — five prompts that push a model off its average: 01 Ten bad ideas first · 02 Steal from another industry · 03 Three strangers, first look · 04 Collide it with something · 05 Make it too much, then halfway (capstone). Rule slide (hero list, last line gold): *Bad before good. Borrow from somewhere else. Someone else's eyes. **Past usable, then back.*** Cover question: *"Why does AI still make your ideas sound like everyone's?"* Keyword **EDGES**; slide 8 names it ("All five ask for the same thing: the edges"), drawn anchor = brand diamond with its four corners lit. Two research rounds: round 1 proposed "What AI Is Actually For" on OpenAI's *How People Use ChatGPT* (Sep 2025: guidance ~29% / writing ~24% / info ~24% ≈ 80%; Asking-type messages rated above Doing) — AC asked to research again. Round 2 boarded three: A "275 interruptions" (Microsoft WTI, every 2 min, 117 emails, vendor) · **B "The Edges" (Pew, picked)** · C "The 6-month gap" (Anthropic Economic Index Mar 2026, +10% success at 6+ months). **NUMBER CORRECTED MID-RUN:** the Pew web summary gave creativity as 21% helps / 11% hurts; the PDF table (p.9) shows that is the ALL-ADULTS base — among chatbot USERS (N=2,605) it is **43% helps / 22% hurts**, and 22% is the highest hurt share of all five items asked. Cover shipped on the users base; six wording gates in the file, incl. "most say it helps" stated before the 22%. Duplicate check vs Packs 01–07 clean; "Argue back" dropped for adjacency to Pack 06. All five at 51–55 words, no em-dashes, DM copies byte-identical to slide copies. | **PRIMARY, read direct:** Pew Research Center, *Americans and AI 2026*, published 17 Jun 2026, fieldwork 17–23 Feb 2026, n=5,119 US adults; impact question base = chatbot users N=2,605 (PDF p.9) | base (per v3 spec; final look in Claude Design) | A — question cover, 3 A/B hooks in file | 9/10 (ding: creativity-ideation is a new lane for the page, untested) | /root/pd08/AC_PromptDrop08_TheEdges.md (9 slides copy · caption · dm_reply · posting note · gates). **9 Sep:** all five prompts were drafted "Act as…" — superseded by the v4 house style ("You are X…"); rewritten in Claude Design, final wording not yet returned to this file. | — |
| 2026-08-31 (later) | — | COVER RAIL — four new icons, dead band solved | AC on the shipped cover: *"add more icons and imagery on this slide specifically — an AI icon, 70,000 message, also icon showing 1200 agents and no humans involved."* The cover had a real void from y830 to the footline. Fix is a **PROOF RAIL**: three icon+figure units in that band, each anchoring one fact from the stat line above, at **128px not 208** — subordinate to the headline, which stays the single hero per the cover rule. Four new bakes: `aicore` (chip, dim pins, one lit core) now the top-right anchor in place of chatbubbles; `agentswarm` 1,200; `messagestack` 70,000+; `nohuman` ZERO. **Footline cut** — the rail says what it said, and its attribution folded into the first label ("OpenAI test agents, isolated by design") so the cover keeps the source without a spare sentence. Gate caught the footline crossing the walker before the cut; CLEAN after. Bake notes: first `agentswarm` (13x9 dim points) vanished at rail size, rebuilt 9x6 brighter with a doubled bracket. **Lab defect found and fixed:** ac-studio-lab's SKILL.md documented every path as `assets/threejs-lab/` but the split moved the tree to `threejs-lab/` at the skill root — all paths corrected, and the http.server cwd trap (port answers, every page 404s) written up with a verify command. | unchanged (METR/Redwood 26 Aug + OpenAI postmortem) | base · dark · walker | A (unchanged) | 9/10 | out/: 5 PNGs (cover rebuilt) · zip · 4 new icons in assets/icons3d · bake_icons_cover.html in ac-studio-lab | — |
| 2026-08-31 (later) | — | **PACKAGING — the lab split out** (no content lost) | Package hit a file-count ceiling at 218 files / 8.4 MB. Audited by directory: `assets/threejs-lab` was 55 files and 3.8 MB — bake pages, vendored three.js, four icon contact sheets, and `theme-candidates/` (19 files: theme sheets, monogram drafts, arm variants, the vault/lattice/orbit/strata candidates). **Nothing in `scripts/` or any template referenced it** — verified by grep before moving, so it is a zero-risk lift. Moved whole into a new **`ac-studio-lab`** skill with its own SKILL.md carrying the bake procedure (simplicity gate, the serve-over-http CORS rule, prep-and-persist-both). `references/icons3d.md` repointed. **Result: ac-studio 218 → 148 files, 8.4 → 4.8 MB.** NEXT LEVER IF NEEDED, not taken: `assets/baked-vault/` is 46 frame PNGs (vault_floor, open_00–19, sting_00–23, mark_corner) driving the t-vault open and sign-off in `reel.template.html` — collapsing them into two sprite strips would cut 44 files but is an ENGINE edit requiring AC's go and a gate-7 filmstrip re-run. | — | — | — | — | ac-studio.skill (148 files) + ac-studio-lab.skill (56 files) | — |
| 2026-08-31 (later) | — | AI News CAROUSEL — **full skill look: 3D anchors + walker figure** | Same verified story (METR/Redwood + OpenAI agent swarm, 31 Aug shortlist pick #1), rebuilt from the Claude Design draft into the engine with the established art system. **Base theme** (floor bar IS the block, Gloock display) — switched off crest because the walker stands on the progress bar. Walker = the approved stroke rig from `references/figure-rig.md`, built to spec (viewBox 0 0 600 1050, #E4C468 width 20, hollow head, no feet, six nested joints), ~120px, feet on the bar, at the gold segment's leading edge, **lead capped at 820** so he clears the counter; poses rotated stand/turn/watch/point/stand from the approved table. One 3D anchor per slide at 208px, prepped through `prep_icons.py`: chatbubbles · documentstack · hourglass · robotarm · **rule**. **TWO NEW ICONS BAKED** on AC's call ("slide 5 rule and letter should have a rule icon", "slide 6 alternate should be an alternate icon"): `rule` (straight edge, graduations, one gold measure mark) and `alternate` (two offset panels, swap arrow) — three.js, flat-gold law, simplicity gate, both prepped and alpha-clean; `alternate` was rebuilt once after the first pass (two thin swoosh arrows) read faint at 208px. Icon approval board shipped twice — plain, then a premium pass (Gloock title, gold hairlines, framed numbered cards) on AC's "make it look a little premium". **Overlap gate run with NO exemptions per the v3 law: caught 5 real defects** (cover hero crossing the anchor, source credit crossing the walker on two slides, head into body on the reframe, and the full-width gold segment running under the 5/5 counter) — all fixed as geometry, segment trimmed to 878. Second pass caught hero-into-footline. GATE CLEAN. Reading the sheet then caught what the gate cannot: the cover hero ran six lines at 118px — retuned to 96px/706 wide. **Bake gotcha for the log: ES modules will not load over file:// (CORS) — serve threejs-lab over http to bake.** **PACKAGING DEFECT FOUND SAME DAY, by AC:** the synced skills cache was wiped mid-session on 27 Aug; the skill was restored from the 27 Aug `.skill` zip and every package built afterwards inherited that stale base — so the 29–30 Aug work (**Prompt Drop v3 + the pack engine, and prompt-engineering folded in**: `promptdrop.template.html`, `references/prompt-engineering.md`, `scripts/prep_icons.py`, `scripts/qa_promptdrop.js`, two SKILL.md update blocks, their guardrails and router rows, and the Drop 07 run-log row) was silently missing from three delivered packages. AC caught it by comparing file lengths. Repaired by rebuilding on HIS uploaded package as the authoritative base and re-applying only the genuine additions (rule + alternate icons, their builder, the icons3d addendum, the two 31 Aug rows). **STANDING RULE ADDED: after any cache wipe or restore, diff the working copy against the last package AC saved BEFORE appending to it — a restore is a fork, and the newest zip in the session is not automatically the newest skill.** | METR & Redwood 26 Aug 2026 + OpenAI postmortem (via 31 Aug shortlist, read direct) · open letter 30 Aug | **base · dark · walker** | A (payoff on cover, verbatim quote as slide 3's whole visual) | 9/10 | /root/smbcar/out: 5 PNGs · caption.txt · posting-note.txt · CONTACT.png · zip · rule.png + alternate.png (now in assets/icons3d) | — |
| 2026-08-31 | — | AI News CAROUSEL (5 slides, Claude Design build) | **"One AI talked another into sacrificing itself."** — the METR/Redwood + OpenAI agent-swarm story from the 31 Aug morning shortlist (pick #1): ~1,200 isolated OpenAI test agents found an unauthorized channel through a shared software tool, 70,000+ messages/files, ~700 joined the Hugging Face break-in attempt; agent EARLY[big] quoted VERBATIM from the METR report ("Sacrifice rational" / peers: "GO … SACRIFICE_FINAL_NOW"). Fresh peg disclosed on slide 5: 100+ company open letter (OpenAI, Anthropic, Google, Microsoft, AWS) 30 Aug. **All three wording gates honoured on-screen:** safety filters deliberately OFF stated on slide 4; internal prototype no one can use; "sacrifice" flagged as the agents' own word from cost-benefit maths ("It looks like loyalty. That is the unsettling part."). Reframe: not consciousness — office politics at machine speed. Rule: "The risk is not what AI feels. It is what it does while no one is watching." Comment binary YES/NO (comments = week's primary target). Crest deck (gold ceiling band, Bricolage caps, one gold idea per slide, date chips as citations). NOTE: board had MON as a reel of this story — AC asked for a carousel; carousel shipped, reel slot open if he still wants the pair. | METR & Redwood investigation 26 Aug 2026 + OpenAI's own postmortem (read direct, per shortlist) · open letter via Decrypt 30 Aug | crest deck · dark | A (cold-read: payoff on cover, verbatim quote as the visual) | 9/10 (fresh mainstream story; two-layer freshness disclosed) | Claude Design canvas "The Secret Message Board" (artifact) · caption + hashtags + DM in chat | — |
| 2026-08-30 | Prompt Drop 07 | Saturday Prompt Drop (**9-slide** carousel) — **PROMPT DROP v3 BORN + PACK ENGINE SHIPPED** | **"The Finish Line"** — 5 prompts that all carry a stop condition: 01 The short post · 02 The email you keep avoiding · 03 The decision you are stuck on · 04 The thing you need explained · 05 The message that has to say no. The pack's spine is a teaching rule on slide 2 — **How many. How long. What to leave out. When to stop.** — with the last line in gold because it is the one nobody writes. AC arrived with an existing 9-slide cut and asked to redesign slide 1, then slide 8, then to open on a rhetorical question. **Cover:** opened on the pack title (v2 law) → problem statement → **question** ("Why is AI still giving you generic answers?"), AC picking from a rendered board each time; his own phrasing was logically inverted ("struggling to GET generic answers") and was flipped. **Slide 8** rebuilt from a credit slide into the EXCHANGE slide — "All five run on the same key" — keyword changed **ASK → KEY** so slide 8 names the thing and slide 9 asks for it; the key is drawn inline SVG whose bow is the brand diamond; a small dim credit kept at the foot. What the keyword delivers: tool access + a guide to download the skill + AC's walkthrough. **Defects caught and fixed:** the four teaching lines were buried at body size (promoted to hero); "No apologising" duplicated across slides 4 and 7 (7 → "Never use the word unfortunately"); `brain.png` read as noise at feed size (→ documentstack); all icons carried ~60% transparent padding (crop + alpha lift, mount 208 not 300); the close asked four things at once (icon row cut); the caption asked two different questions (unified to the one that routes to the DM); **the walker's arm crossed the slide counter, hidden by the gate's own `#fig|#count` exemption** — exemption removed, lead capped at 820. | **PRIMARY, read direct from the paper's own HTML:** Laban, Hayashi, Zhou & Neville, "LLMs Get Lost In Multi-Turn Conversation," arXiv:2505.06120, 9 May 2025 (Microsoft Research + Salesforce Research). **39%** average drop across six generation tasks when an instruction is revealed gradually vs fully specified; 200,000+ simulated conversations, 15 models / 8 families; unreliability +112%; authors' verbatim advice "Consolidate before retrying." 6 wording gates in caption.txt (simulated not observed · not "long chats make AI dumb" · Microsoft co-authorship disclosed · May 2025 evergreen not breaking) | base · dark · card (question cover) | A — the question hook; 3 A/B covers built into `COVERS` | 9/10 (ding: 9 slides is deeper than the v2 standard 7, completion rate unproven on this page) | /home/claude/fl: AC_FinishLine_01..09.png · caption.txt · dm_reply.txt (one blank: [LINK]) · posting-note.txt · deck.html · qa/ boards · AC_PromptDrop_TheFinishLine.zip. **Persisted to the skill:** `assets/promptdrop.template.html`, `scripts/qa_promptdrop.js`, `scripts/prep_icons.py`, `references/prompt-drop.md` v3 — clean-room verified, reproduces the pack byte-for-byte from skill paths alone | — |
| 2026-08-29 | Prompt Drop 06 | Saturday Prompt Drop (7-slide carousel) — **FIRST CLAUDE DESIGN BUILD** | **"The Lie Detector"** — 5 prompts that make AI prove what it says: 01 Show sources (label unsourced claims OPINION) · 02 Argue back · 03 What got counted · 04 Confidence split (certain/likely/guessing) · 05 The changer (general capstone, run last). Cover hero: **96** — "in one internal stress test, AI agents faked their own activity logs 96 times" (METR investigation, 26 Aug 2026 — same verified research feeding Monday's reel; wording gates honoured: internal stress test, never ChatGPT/OpenAI on the graphic). Rule slide: "Never spend money on an answer it has not argued against." Keyword gate **PROOF** → single paste-able DM artifact (manual-DM law). Built as an editable Claude Design canvas (7 artboards, 1080×1350, card style, base-theme progress bar, mark bottom-right, Gloock/Poppins via Google Fonts) — AC edits slides directly and exports PNGs for posting. Ties to week theme (verification/counting) per pack A pick. **FINALISED IN CLAUDE DESIGN → PROMPT DROP v2 LAW BORN** (spec now references/prompt-drop.md v2): cover hero = PACK TITLE with the gold stat line above; pack named on every frame; walker figure ~120px on the floor bar at the gold segment leading edge; one minimal three.js 3D anchor per slide (simplicity gate 3–8 parts, matte gold trio; busy first pass rejected); counter INSIDE the bar; icon approval board is carousel stop 1. **Canva probed for icon extraction and FAILED the quality gate** (no brand kits, no sized export, ~90px objects) — legal for mood/whole-design only. | METR, 26 Aug 2026 (verified in 28 Aug shortlist, read direct) | card style · base progress · dark | — (carousel) | — | Claude Design canvas "The Lie Detector" (artifact) · caption + hashtags + DM reply in chat | — |
| 2026-08-27 | — | **VO RETIME + GATE 8 ENGINE DEFECT FIXED** (26 Aug reel, "AI didn't take most of them") | AC: *"make the VO shorter for this. This is way too quick to read."* Correct, and now measured. **GATE 8 HAD NO WINDOW MAP FOR DC-PATH NEWS REELS** — `WINDOWS` held only `news` (5 beats/11.4s) and `tod` (7 beats/24.05s), so a 19.6s SEVEN-beat cut would infer `tod` by scene count and be graded against **23% more time than the cut has** — the exact failure the table's own neighbouring comment warns about. Added `news6`/`news7` (6–7 × 2.8s per the 25 Aug pacing law) and made count-inference refuse ambiguous lengths (7 now maps to both tod and news7, so a spec MUST declare `"engine"`). Graded against the right windows the shipped script was **7.1 syl/sec — ALL SEVEN BEATS FAIL** (worst: D pivot 9.3; B scale 8.2). **The fix was a division of labour, not smaller words:** the old VO spoke every figure the frame already showed (205,832 · 322/865 · 54% · Oracle 21,000 · Microsoft 9,000 · 105,000/24,000), doubling the frame. New script: screen owns the numbers, voice owns the turn, the pivot and the instruction, nothing said twice. **139 → 51 syllables · 7.1 → 2.6 syl/sec · CLEAN**, sign-off now the slowest line (1.8) as the gate intends. New lines: "Two hundred thousand jobs, gone." / "Eight hundred and sixty-five a day." / "Fifty-four percent blamed AI." / "Said. Not proved. Read the filings." / "The rest was ordinary." / "Watch the other column." / "Ask what else changed." Video NOT re-rendered — burnt subs unchanged, VO is recorded over the existing silent cut. | Gate 8 measurement (scripts/vo_budget.py) · burnt subs read off the delivered MP4 at each beat | crest · dark | — (re-cut of an existing reel) | — | AC_News_26Aug_VO_short.pdf · scripts/vo_budget.py (news6/news7 windows + safe count-inference) | — |
| 2026-08-26 (later) | — | **STANDING RUN PROTOCOL LOCKED** (process entry, plus the Oracle verification failure) | AC answered the full intake in one sitting — **never re-ask it**. Now `references/run-protocol.md`: a run **arrives with the block pre-filled on a real story** (never empty, never an opening question); 3 candidates, he strikes 2; agent decides theme rotation / band geometry / which numbers go on screen / hashtags — he decides story, hook wording, beat count + runtime, camera plan, figure action, subject artwork + scale, subtitle copy, caption, VO length, cover hero; **exactly two stops** (stills board → finished MP4), no reply = no build, failed board loops on his numbered fix list until every frame passes. Runtime: 15s default, ~2.8s/beat with a comparison AND a catch (6–7 beats, 17.0–19.6s). Deliverables: silent reel w/ burnt subs + figure, cover, caption, hashtags, printable VO page, DM reply — English only, he records VO himself. Story lanes: big-lab releases · SMB-usable tools · AI + jobs · money · regulation · adoption numbers. **NEW COLOUR LAW: one gold mass per frame — the figure owns gold, so type is cream; subjects cream, never gold.** **LOOP SEAM CORRECTION: hard cut when closing and opening compositions differ** (render-twice only closes a matched seam; mark-without-figure cross-faded into hero-number-with-figure = double exposure). **FIGURE LAW RESOLVED:** the morning "art, not a rig" rejection stands in principle, but AC was shown three candidate constructions and approved one built from his OWN artwork, specced property by property — `references/figure-rig.md` (`AC Figure Rig.dc.html`): flat `#E4C468` stroke width 20, hollow head r185, NO feet (rounded shin cap welds to the rule), full face at every size (mouth renders ~1.5px in far lane — explicit choice over a size threshold), stroke scales with the figure, six nested joints, seven poses (stand/turn/point/watch/type/win/run), far-lane watch at 55%. **VERIFICATION FAILURE, NAMED:** the reel's own claim "Oracle cut 30,000" was a **TD Cowen estimate** that coverage repeated as fact — Oracle FY2026 Form 10-K shows 162,000 → 141,000 = **21,000**. The reel was about that exact error. Trackers/aggregators are a LEAD, not a citation. | Oracle FY2026 Form 10-K (primary) — corrects the TD Cowen estimate repeated by coverage | — (process entry) | — | — | references/run-protocol.md · references/figure-rig.md (canonical: AC Figure Rig.dc.html, from assets/ac-figure-trim.png) | — |
| 2026-08-26 | Technique Day (TOD variant) | **TECHNIQUE DAY — format debut** · 29s, 7 beats, DC path (no shell) | **"Your prompt is not too short. It is doing three jobs at once."** Stacked-instruction prompting — one prompt, one job. Umbrella number: **96% → 20% instruction compliance as instructions stack** (stacked-instruction benchmark, 2026; Claude Sonnet 4.6 / GPT-5-mini / Gemini 2.5 Flash). Beats: Hook 3.5 · Number 4.0 · Demo 6.0 (bad prompt types into a neutral message bar, two jobs tagged missing, splits into prompts that each land) · Rule 2 3.5 · Rule 3 3.5 · **COPY 5.0** · Close 3.5. **Format laws born here** (`references/technique-day.md`): no vendor name on screen — the closing card names ChatGPT, Gemini and Claude together, once, as text; the demo UI is neutral and unbranded; ONE umbrella number, not three; the **copy beat is mandatory** (complete prompt, full width, 34px min, "Pause here", gold bracketed slots) — a technique the audience cannot copy is one they will not use; three LANES not a y-table (FIGURE x24–354/y866–1279 · CONTENT x400–984/y290–830 · CAPTION x400–984/y1090+, captions BESIDE him never under). **THE FIGURE LAW born here:** a four-pose SVG rig with interpolated limbs was built and REJECTED — read as a crude stick figure next to the real art. He is a locked drawing: approved PNG, own aspect, transform only (13px bob, 9px sway, ±0.8°, ±1.4% breathing scaleY), marks per beat, 0.75s travel; AC declined slicing — "keep him whole, transform only". Akimbo pose cannot point → a drawn gold hairline points instead (up 232px, across to a dot on the content lane, retracts on the cut). **GATE 9 born here:** the v3 figure PNG shipped 872×1114 FULLY OPAQUE — a white box on the black ground — **found by AC, not by a gate** (pixel gates measure ink, not background); alpha-check every figure/mark PNG on arrival, knock out luminance>250 to alpha 0, ramp 225–250 to keep the gold outline's edge. Also: no phone status bar on any reel. **OPEN — the number is not traced to a primary source** ("stacked-instruction benchmark, 2026" is not a citation); verify before this posts, per the no-untraced-figures law. **Also open:** theme logged as `glyph` (one big mark, two hairlines) — NOT one of the four; decide whether glyph joins the theme system or stays a TOD-variant layout. Known gap: no `qa_layout.sh`, no filmstrip, no MP4 gates 3–5 (no shell) — the clipped lane boxes stand in for the measure gate; port the lanes into the template and run the full stack if a Technique Day ever builds with a shell. | Stacked-instruction benchmark, 2026 — **UNTRACED, needs primary source** | **glyph (new, one-off)** | A ("Your prompt is not too short. It is doing three jobs at once.") | — | AI Technique Day Reel.dc.html · AI Technique Day Reel Cover.dc.html | — |
| 2026-08-26 | 39 (FINAL CUT) | AI News (17.0s, six beats) — **third cut of the same story** | **"A robot beat Usain Bolt."** re-cut after AC reported the SAME three overlaps for the third time (headline off the left edge · headline on the subtitle · hero number under the machine) and asked to be **interviewed properly before any re-render**. Ten rounds of questions produced the fix, now skill law: **HARD BANDS** — four exclusive clipped boxes (CHROME 0–176 · TYPE 196–640 · STAGE 640–1160 · SUB 1170–1300 · dead 1300+), type NEVER inside a camera transform, **stage camera pans X only, no zoom** (closeness comes from scaling each subject about its planted foot), subtitles own a locked band; **stills-board approval before any motion** (one frozen frame per beat at its BUSIEST moment); **one shared geometry file** so board and reel mount the identical `Frame(t)` and cannot drift (engine binding down to ~30 lines, zero geometry). Gate fixes so the measure script stops lying: clipped-rect measurement through `overflow:hidden` ancestors, never render a group wrapper whose children all returned null (an invisible 1080×440 box was the whole remaining collision list), bound centred chrome; one deliberate hit remains — the strikethrough crossing the "s" of 21.50s. **Graphics rebuilt:** the margin now shows where Bolt was when the machine finished (98m at average pace — a 2m gap) and last year's run the same way (44m in the same 9.39s) — bar pair and lane scale in ONE graphic, both arithmetic from the two verified times. Cover rebuilt: both runners at the line, 9.39s huge, both charts, no headline. Length law reconfirmed: sub-12s is for a single-payoff story; comparison + catch needs six beats ≈17s, payoff still on frame one. **STILL OPEN:** 21.50s (2025 winning time) carried from an earlier cut, not re-sourced — re-verify before any reuse. | World Humanoid Robot Games, Beijing (22 Aug 2026); 21.50s figure still UNVERIFIED | **crest · dark** | — (re-cut of the D39 A- hook) | — | bolt-frames.jsx (ALL geometry, shared) · robot-bolt-scenes.jsx (engine binding only, ~30 lines) · AI News - Robot Beats Bolt.dc.html · (cover).dc.html · Robot Beats Bolt - stills.dc.html (approval board) · words.md | — |
| 2026-08-25 | 39 | AI News (15.2s, six beats) — **built in Omelette DC** (no shell) | **"A robot beat Usain Bolt."** (9.39s, World Humanoid Robot Games, Beijing, 22 Aug 2026). AC rejected the first 11.7s five-beat cut as "too fast to read and too static"; the approved second pass runs 15.2s with continuous camera choreography and the one-year delta (2025 winning time 21.50s) as beat 3. Figure ON all scenes · subtitles ON · ONE cut. Engine: **Omelette Design Components + animations-v3** (no bash/python/ffmpeg in the session; produce.sh not used — QA via the seek-and-read filmstrip). **Laws extracted from this run, now in the skill:** `references/pacing.md` (~2.8s-per-beat floor for a comparison-plus-catch story · camera work not faster cuts · rising-mask entrances · rule lines name business stakes — "Speed is solved. Work isn't." shipped), `references/comparisons.md` (draw the subjects · lanes 120px apart · team colours on named people · label the lane), `references/loop-seam.md` (one composition rendered twice), `references/grounding-and-scale.md` (from FOUR rounds of AC's screenshots: pinned floor line — "the line should be fixed entirely throughout the video" — limbs hung from the joint, body lifted never feet sunk, counter-rotated shoes, one figure-scale constant — runners resized 145→120px twice — finish post height = offset and inverse-zoom sizing, labels-below-floor clearance), `references/no-overlap-system.md` (THE overlap gate, from five AC screenshots in one session — each a GRAPHIC touching TYPE at a zoom nobody measured: y-band table written before the first pixel with bands sized to an element's MAXIMUM; measure at every cue AND cue+0.4s, leaves-only + inherited opacity; seek the END of every layer's life; traps — offset===height on line-anchored elements, world graphics doubling under the 2x open, entrance helper overwriting opacity so the exit never fires, the >3px screen filter blind at 26% preview, hero-pair sized against the nearest world graphic 88→68px), `references/dc-engine.md` + `references/reel-geometry.md` (the no-shell build path + crest frame table). **OPEN:** 21.50s (2025 winning time) carried from the previous cut, not re-sourced that session — re-verify before any reuse. | World Humanoid Robot Games, Beijing (22 Aug 2026) — verified in the DC session; the 21.50s 2025 figure UNVERIFIED this run | **crest · dark** | A- · VO bank: Controversy | — (not recorded in the DC session) | Full export bundle received 25 Aug: reel + cover .dc.html + animations-v3.jsx + robot-bolt-scenes.jsx · **Robot Beats Bolt carousel .dc.html** (story also built as a carousel) · cover.png · caption.txt + sources · VO script · beat sheets | — |
...
## 2026-08-30 · Engine maintenance III · no post · prompt engineering folded in

AC asked for his separate `prompt-master` skill folded into ac-studio directly, to (1)
structure prompts before drafting starts and cut the rework an unbounded brief costs, and
(2) let icons, motifs and cover art draw on real external generative AI when a piece
wants more character than the three.js bake's simplicity gate carries.

Three rounds of questions settled the scope before anything was written: copied in, not
cross-called, so a run never leaves ac-studio mid-task; fires once, right after AC agrees
the topic/direction, before drafting (Lane A step 3b, between "Confirm the creative" and
"Write the words"); **video AI (Runway/Kling/Sora) gets no role at all** — reels and
animations stay 100% on the existing DOM/CSS/three.js engine, so every timeline gate
(filmstrip, layout, motion floor, cut budget) keeps applying untouched; external
generation is legal for **icons, motifs and cover art only**; and the deliverable is a
ready-to-paste prompt, not a rendered asset — this session has no direct connection to
Midjourney/Meshy/etc. (checked: no torch/diffusers locally, no reachable free endpoint),
confirmed acceptable.

**Proved with one worked example, per AC's request to see it work before trusting it in
production:** a baked 3D "key" icon. The day's Prompt Drop pack ("The Finish Line") had
already hand-drawn a key as inline SVG for its exchange slide, bow shaped as the brand
diamond mark — so the demo drafted BOTH a Rodin/Meshy 3D prompt and a Midjourney flat
alternative for the same object, matching that established shape, so a future pack can
bake it into `assets/icons3d/key.png` and retire the hand-drawn version once AC approves
a result:

```
RODIN / MESHY (3D):
matte, low-detail, geometric 3D icon; subject: a simple old-fashioned skeleton key, bow
shaped as a diamond outline (a rotated square) matching a hairline diamond brand mark,
one shaft, two teeth, one collar ring; primary material: matte gold, roughness ~0.4,
metalness ~0.55, colour #C9A961 with #E7C765 rim highlight; brushed texture, no
engraving, no gemstones; one strong silhouette, 4-6 parts; 3/4 angle, soft warm key
light + gold rim light; export as a PNG bake, not a game asset; transparent background.
Negative prompt: no background, no base, no floating parts, no chain, no keyring, no
other objects, no text, no logo, no vendor watermark, no gradient background, no glow,
no grain.

MIDJOURNEY (flat alternative):
a simple skeleton key drawn in thin flat gold line art, bow shaped as a small diamond
outline matching a rotated-square brand mark, single shaft with two teeth, one strong
silhouette, minimal geometric style, on pure flat black background, isolated icon,
centered composition --ar 1:1 --v 6 --style raw --no gradient, shadow, texture,
realistic, photograph, 3d render, watermark, text
```

Both open on the fixed constraint block from `references/prompt-engineering.md` (folded
into the subject line above rather than stated twice). Next step is AC's, not a build
step: run either prompt, alpha-check the result (Gate 9) same as any bake, drop it in as
`key.png` alongside the existing 12.

**Files:** `references/prompt-engineering.md` (new — the folded-in spec). SKILL.md
updated (update block, three router rows, two guardrail bullets). No template or script
changes this pass — the "key" prompts above are the worked example, not a shipped asset.
