#!/usr/bin/env bash
# AC — AI News · one-shot build: render frames -> synth audio -> encode+mux.
# Run from a WORKING copy of the assets (never edit the skill's originals in place).
#   ./build_video.sh <reel.html> <out.mp4> [--silent]
# Produces the sub-12s MP4 with synced sound (unless --silent). Cleans up frames after.
set -euo pipefail
HTML="${1:-ainews.template.html}"
OUT="${2:-AC_AINews.mp4}"
SILENT="${3:-}"
...
if [ "$SILENT" == "--silent" ]; then
  ...
  python3 "$ASSETS/ainews_audio.py" "$WORK/master.wav"
